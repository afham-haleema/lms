<x-guest-layout>
    <!-- Session Status -->
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 2000)" class="mb-4 mx-2">
        <x-auth-session-status :status="session('status')" />
    </div>

    <form method="POST" action="{{ route('login') }}" class="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[350px] ">
        @csrf

        <div class="flexflex-col space-y-2">
            <h1 class="text-center font-semibold text-2xl tracking-tight">Login to your account</h1>
